...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { token: newRefreshToken, jti: newJTI } = signRefreshToken(user._id.toString());

    user.refreshTokenJTI = newJTI;
    await user.save();

    const accessCookie = `access-token=${newAccessToken}; HttpOnly; Path=/; Max-Age=900; ${isProd ? "Secure; SameSite=Lax;" : ""
      }`;
